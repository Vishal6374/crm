...
export default function AttendancePage() {
  ...
  const fetchAttendance = useCallback(async () => {
    ...
    const { data: attendanceData, error } = await builder;
  ...
  async function createAttendance(e: React.FormEvent) {
    e.preventDefault();
    if (editing) {
      if (!can("attendance", "can_edit")) {
        toast({ title: "Not allowed", description: "You do not have permission to edit attendance.", variant: "destructive" });
        return;
      }
      ...
      if (!can("attendance", "can_create")) {
        toast({ title: "Not allowed", description: "You do not have permission to record attendance.", variant: "destructive" });
        return;
      }
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {!can("attendance", "can_view") ? (
        <div className="text-sm text-muted-foreground">You do not have permission to view attendance.</div>
      ) : (
        <>
